...
- **Blessing movies rebuilt with all 11 blessings** (2026-07-05): `Hero-Movie-Long.mp4` (8:45) + `Hero-Movie-Short.mp4` (4:16). `blessing_segments.json` is now fully curated — 6 segments auto-found by faster-whisper (`medium` model; `small` misses most Hebrew), 5 hand-picked from the word-level transcripts because those greeters never say "מזל טוב" verbatim (they bless differently) or whisper garbled it. Word-by-word transcripts of all 11 blessings were reviewed to pick natural moments — DON'T re-run `--transcribe` blindly; it only fills `null` entries, so the curated values are safe, but resetting them to null loses the curation.
