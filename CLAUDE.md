...
- **Hero-Movie.mp4 updated** (2026-07-05): step-7 scene is now the real soccer clip (`videoscene()` in make_movie.py — slow-motion loop, transparent HTML caption overlay composited by ffmpeg) instead of a still; the step-3 ultrasound caption + Quest 3 quiz step + storybook note were reworded to remove the triplet-pregnancy reference ("הפתעה כפולה" framing) — per the parents, that detail must not appear anywhere.
